...
void Project::mount_project(std::string projectName) {
  ...
  try {
    ...
    // Log success before checking for files, as directory creation is
    // successful
    ...
    if (!AgniVault::fetchKeyFromConfig(key)) {
      LOG_DEBUG_WARN(
          "Key Not found in config; To generate new key, RUN: agniv gen-key");
      sqlite3_close(db);
      return;
    }
    std::string name;
    ...
    if (!AgniVault::getUsernameFromConfig(name) ||
        !AgniVault::getEmailFromConfig(email)) {
    ...
    for (const auto &fileRow : filesToMount) {
      ...
      if (fullEncryptedData.size() < nonceSize) {
        LOG_DEBUG_ERROR("Encrypted data size is less than nonce size for file: "
                        << fileRow.path << ". Skipping.");
        continue;
      }

      std::vector<unsigned char> nonce(fullEncryptedData.begin() +
                                           fullEncryptedData.size() - nonceSize,
                                       fullEncryptedData.end());
      std::vector<unsigned char> cipherBytes(
          fullEncryptedData.begin(), fullEncryptedData.end() - nonceSize);
      std::vector<unsigned char> decryptedData;

      LOG_DEBUG("Cipher Bytes Size: " << cipherBytes.size());

      bool decryptSuccess = encryption.decryptBuffer(
          decryptedData, key.data(), nonce, uname, projectName, cipherBytes);
      LOG_DEBUG(
          "Decrypt Buffer Success: " << (decryptSuccess ? "true" : "false"));
      LOG_DEBUG("Decrypted Data Size: " << decryptedData.size());

      if (!decryptSuccess) {
        LOG_DEBUG_ERROR("Failed to decrypt file: " << fileRow.path
                                                   << ". Skipping.");
        continue;
      }

      std::string fullPath =
          projDir + "/" + fileRow.path; // Construct full path for writing
      ...
      if (!AgniVault::File::writeFile(fullPath, decryptedData)) {
        LOG_DEBUG_ERROR("Failed to write decrypted data to file: "
                        << fullPath << ". Skipping.");
        continue;
      }
    }
